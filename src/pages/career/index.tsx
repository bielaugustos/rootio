      ...
      <div style={{ padding: '16px 18px 12px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 15, color: 'var(--t1)' }}>{goal.title}</span>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', padding: '2px 7px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--border)', background: goal.status === 'done' ? 'var(--main)' : goal.status === 'paused' ? 'var(--bg3)' : 'transparent', color: 'var(--t2)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--t2)', flexShrink: 0 }}>{goal.progress}%</span>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {(Object.entries(CATEGORY_META) as [GoalCategory, typeof CATEGORY_META[GoalCategory]][])
                  ...
                  <button key={key} onClick={() => setCategory(key)} style={{
                    ...
                    border: '2px solid var(--border)', cursor: 'pointer', fontSize: 12, fontWeight: 600,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{ background: 'var(--secondary-background)', border: '2px solid var(--border)', borderRadius: 'var(--radius-base)', boxShadow: '3px 3px 0 var(--border)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, overflowX: 'auto', paddingBottom: 2 }}>
        {([['all', 'Todas'], ['active', 'Em andamento'], ['done', 'Concluídas'], ['paused', 'Pausadas']] as const).map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)} style={{
            ...
            border: '2px solid var(--border)', fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 600, cursor: 'pointer',
...
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--t3)',
