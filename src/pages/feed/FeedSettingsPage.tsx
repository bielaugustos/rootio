    ...
    <PageWrapper maxWidth={600}>
      ...
      {[
        ...
        <div key={section.label} style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--t3)', marginBottom: 8, paddingLeft: 4 }}>
            {section.label}
          </div>
          <div style={{ border: '2px solid var(--border)', borderRadius: 'var(--radius-base)', background: 'var(--secondary-background)', overflow: 'hidden', boxShadow: '3px 3px 0 var(--border)' }}>
            {section.rows.map((row, i) => (
              <div key={row.label} style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', borderTop: i > 0 ? '1px solid var(--b2)' : 'none' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--t1)' }}>{row.label}</div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {['Hábitos', 'Carreira', 'Tech', 'Finanças', 'Mentalidade'].map(c => (
          <span key={c} style={{ padding: '6px 14px', borderRadius: 'var(--radius-base)', border: '2px solid var(--border)', fontSize: 13, fontWeight: 600, background: 'var(--main)', color: 'var(--main-foreground)', boxShadow: '2px 2px 0 var(--border)', cursor: 'pointer' }}>
            {c}
          </span>
        ))}
      </div>
