...
function ProgressBar({ step, total }: { step: number; total: number }) {
  return (
    <div style={{ width: '100%', maxWidth: 400 }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--t3)', textTransform: 'uppercase' }}>
        {step} / {total}
      </div>
    </div>
  )
...
function FeatureRow({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 14, color: 'var(--t1)', marginBottom: 2 }}>{title}</div>
    ...
    body: (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 400 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--t3)', textTransform: 'uppercase', marginBottom: 6 }}>
            POTENCIAL MENSAL
          </div>
          <div style={{ fontFamily: 'var(--font-title)', fontSize: 32, fontWeight: 700, color: 'var(--main)' }}>
  ...
  return (
    <div className="onboarding" style={{
      ...
      <div style={{ width: '100%', maxWidth: 440, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        ...
        <button
          onClick={skip}
          style={{ background: 'none', border: 'none', fontSize: 13, fontWeight: 600, color: 'var(--t3)', cursor: 'pointer', whiteSpace: 'nowrap', paddingTop: 2, flexShrink: 0 }}
        >
          Pular
        </button>
      </div>
