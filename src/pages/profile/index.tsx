...
function StatPill({ label, value, icon }: { label: string; value: string | number; icon: string }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--font-title)', fontSize: 22, fontWeight: 700, color: 'var(--t1)', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 11, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{label}</div>
    </div>
  )
}
  ...
  return (
    <PageWrapper maxWidth={720}>
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 20px 0', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 10px', background: 'var(--main)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)', boxShadow: '2px 2px 0 var(--border)' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--main-foreground)' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--t3)' }}>XP</span>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--t2)' }}>{eco.progresso_nivel}%</span>
          </div>
        ...
        <div style={{ display: 'flex', marginTop: 16, borderTop: '2px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flex: 1, padding: '14px 8px' }}>
            ...
            <div style={{ fontFamily: 'var(--font-title)', fontSize: 14, fontWeight: 700, color: 'var(--t1)', lineHeight: 1 }}>{memberSince}</div>
            <div style={{ fontSize: 11, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>Membro</div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: profile.plan === 'pro' ? 'var(--background)' : 'var(--t1)', marginBottom: 2 }}>
        ...
        {profile.plan === 'free' && (
          <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', background: 'var(--main)', color: 'var(--main-foreground)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)' }}>
            PRO →
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
        {[
          ...
          >
            ...
            <div style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 14, color: 'var(--t1)' }}>{link.label}</div>
