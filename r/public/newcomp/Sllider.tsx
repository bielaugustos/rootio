...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div style={{ marginBottom: 40 }}>
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: "#999", textTransform: "uppercase", marginBottom: 28 }}>
        {title}
      </p>
...
export default function SliderDemo() {
  ...
  const [v2, setV2] = useState<[number, number]>([20, 70]);
  const [vol, setVol] = useState(50);

  return (
    <div className="sl-root" style={{ background: "#F5F3EE", minHeight: "100vh", padding: "40px 32px", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{CSS}</style>
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        <h1 style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", color: "#999", textTransform: "uppercase", marginBottom: 40 }}>
          Slider · variantes
        </h1>

        <Section title="Basic">
          <Slider label="Continuous slider" defaultValue={30} valueLabelDisplay="auto" />
        ...
        <Section title="Sizes">
          <Slider label="Small" size="small" defaultValue={40} valueLabelDisplay="auto" color="secondary" />
          <Slider label="Medium (default)" defaultValue={60} valueLabelDisplay="auto" color="primary" />
        </Section>

        <Section title="Marks">
          <Slider label="Auto marks" defaultValue={20} step={10} marks valueLabelDisplay="auto" />
          <Slider
        ...
        <Section title="Range slider">
          <Slider
            label={`Range · ${v2[0]} – ${v2[1]}`}
            value={v2}
            onChange={(v) => setV2(v as [number, number])}
            valueLabelDisplay="auto"
            color="success"
          />
          <Slider
        ...
        <Section title="Colors">
          {(["primary","secondary","error","info","success","warning"] as const).map((c) => (
            <Slider key={c} label={c} defaultValue={40 + Math.random() * 30} color={c} valueLabelDisplay="auto" />
          ))}
        </Section>

        <Section title="Value label always on">
          <Slider label="Volume" value={vol} onChange={(v) => setVol(v as number)} valueLabelDisplay="on" color="primary" />
        </Section>

        <Section title="Custom label format">
          <Slider
            label="Temperature"
            defaultValue={37}
            valueLabelDisplay="on"
        ...
        <Section title="Inverted track">
          <Slider label="Inverted" defaultValue={40} track="inverted" valueLabelDisplay="auto" color="warning" />
        </Section>

        <Section title="No track">
          <Slider label="No track" defaultValue={55} track={false} valueLabelDisplay="auto" />
        </Section>

        <Section title="Steps">
          <Slider label="Step = 10" defaultValue={40} step={10} marks valueLabelDisplay="auto" color="secondary" />
          <Slider label="Step = null (free)" defaultValue={33.3} step={null} valueLabelDisplay="auto" color="primary" />
        </Section>

        <Section title="Vertical">
          <div style={{ display: "flex", gap: 48, alignItems: "flex-end" }}>
            <Slider orientation="vertical" height={180} defaultValue={30} valueLabelDisplay="auto" color="primary" />
            <Slider orientation="vertical" height={180} defaultValue={[20, 70]} valueLabelDisplay="auto" color="success" />
            <Slider orientation="vertical" height={180} defaultValue={50} step={10} marks valueLabelDisplay="auto" color="secondary" />
            <Slider orientation="vertical" height={180} defaultValue={60} disabled />
          </div>
        </Section>
