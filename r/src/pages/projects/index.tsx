          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {(['baixa','media','alta'] as const).map(p => (
                <button key={p} onClick={() => setPriority(p)} style={{ flex: 1, padding: '6px 0', borderRadius: 'var(--radius-sm)', border: '2px solid var(--border)', cursor: 'pointer', fontSize: 12, fontWeight: 500, background: priority === p ? PRIORITY_COLOR[p] : 'var(--secondary-background)', color: priority === p ? '#fff' : 'var(--t2)', boxShadow: priority === p ? '2px 2px 0 var(--border)' : 'none', textTransform: 'capitalize' }}>
                  {p}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {(Object.entries(COL_META) as [KanbanCol, typeof COL_META[KanbanCol]][]).map(([k, meta]) => (
                <button key={k} onClick={() => setCol(k)} style={{ flex: 1, padding: '6px 0', borderRadius: 'var(--radius-sm)', border: '2px solid var(--border)', cursor: 'pointer', fontSize: 12, fontWeight: 400, background: col === k ? 'var(--main)' : 'var(--secondary-background)', color: col === k ? 'var(--main-foreground)' : 'var(--t2)', boxShadow: col === k ? '2px 2px 0 var(--border)' : 'none' }}>
                  {meta.label}
                </button>
              ))}
            </div>
            ...
            <div key={col} style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 200 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--t2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{meta.label}</span>
                  <span style={{ fontSize: 11, fontWeight: 500, background: 'var(--bg3)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '1px 6px', color: 'var(--t3)' }}>{colCards.length}</span>
                </div>
              ...
              {colCards.map(card => (
                ...
                >
                  <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--t1)', marginBottom: 8, lineHeight: 1.4 }}>{card.title}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    {card.tag && (
                      <span style={{ fontSize: 10, fontWeight: 500, padding: '1px 6px', borderRadius: 'var(--radius-sm)', border: '1.5px solid var(--border)', background: 'var(--bg2)', color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{card.tag}</span>
                    )}
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, overflowX: 'auto', paddingBottom: 2 }}>
          {projects.map(p => (
            <button key={p.id} onClick={() => setActiveId(p.id)} style={{
              ...
              fontWeight: 400, fontSize: 13, transition: 'all 0.12s',
            }}>
...
const lbl: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }
