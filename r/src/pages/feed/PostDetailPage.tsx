  ...
  return (
    <PageWrapper maxWidth={680}>
      ...
      <div style={{ border: '2px solid var(--border)', borderRadius: 'var(--radius-base)', boxShadow: '4px 4px 0 var(--border)', background: 'var(--secondary-background)', overflow: 'hidden', marginBottom: 16 }}>
        ...
        <div style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
            <div style={{ width: 40, height: 40, borderRadius: 'var(--radius-sm)', border: '2px solid var(--border)', background: post.bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 500, fontSize: 12, color: post.fgColor }}>
              {post.initials}
            </div>
            <div>
              <div style={{ fontWeight: 500, fontSize: 14, color: 'var(--t1)' }}>{post.handle}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 14, borderTop: '1px solid var(--b2)' }}>
            <button onClick={() => { setLiked(!liked); setLikeCount(n => n + (liked ? -1 : 1)) }} style={{ ...actionBtn2, color: liked ? '#ef4444' : 'var(--t3)' }}>
              ...
              <span style={{ fontWeight: 500 }}>{likeCount >= 1000 ? `${(likeCount/1000).toFixed(1)}k` : likeCount}</span>
            </button>
      ...
      <div style={{ border: '2px solid var(--border)', borderRadius: 'var(--radius-base)', background: 'var(--secondary-background)', padding: 16, marginBottom: 16 }}>
        <h3 style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 14, color: 'var(--t1)', marginBottom: 12 }}>Seu Progresso</h3>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <span style={{ fontSize: 13, color: 'var(--t1)' }}>Módulo de Design Digital</span>
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--main-foreground)' }}>75%</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {COMMENTS.map((c, i) => (
            <div key={i} style={{ display: 'flex', gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 'var(--radius-sm)', border: '2px solid var(--border)', background: 'var(--bg3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 500, fontSize: 11, color: 'var(--t2)', flexShrink: 0 }}>
                {c.initials}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 3 }}>
                  <span style={{ fontWeight: 400, fontSize: 13, color: 'var(--t1)' }}>{c.name}</span>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <button style={{ background: 'none', border: 'none', fontSize: 12, fontWeight: 500, color: 'var(--main-foreground)', cursor: 'pointer' }}>Responder</button>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 'var(--sidebar-w, 56px)', right: 0, background: 'var(--secondary-background)', borderTop: '2px solid var(--border)', padding: '10px 20px', zIndex: 40 }}>
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: 'var(--radius-sm)', border: '2px solid var(--border)', background: 'var(--main)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 500, flexShrink: 0 }}>EU</div>
          <input
            value={reply}
