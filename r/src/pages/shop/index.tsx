  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {owned && (
          <div style={{ position: 'absolute', top: 6, left: 6, background: 'var(--main)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '2px 6px', fontSize: 9, fontWeight: 500 }}>
            ✓ POSSUÍDO
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--main)' }}>IO {item.price}</span>
          {!owned && (
            <span style={{ fontSize: 10, fontWeight: 500, color: canAfford ? 'var(--t2)' : '#ef4444' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, overflowX: 'auto', paddingBottom: 2 }}>
        {([['todos','Todos','squares-four'],['avatares','Avatares','smiley'],['temas','Temas','palette'],['powerups','Power-ups','lightning']] as const).map(([k, label, icon]) => (
          <button key={k} onClick={() => setFilter(k)} style={{
            ...
            border: '2px solid var(--border)', fontSize: 13, fontWeight: 400, cursor: 'pointer',
