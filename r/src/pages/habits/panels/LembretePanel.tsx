...
type HabitWithConfig = Habit & { reminder_config?: ReminderConfig }
...
function SnoozeBar({
  ...
  const snoozeOptions = useMemo(() => {
    ...
    const tomorrowMs = tomorrow.getTime() - now.getTime()
    return [
      { l: '+15min', ms: 15 * 60 * 1000 },
      { l: '+1h', ms: 60 * 60 * 1000 },
      { l: 'Amanhã', ms: tomorrowMs },
    ]
  }, [])
...
export function LembretePanel({
  ...
  const [config,  setConfig]  = useState<ReminderConfig>(() => ({
    ...DEFAULT_CONFIG,
    ...(habit.reminder_config ?? {}),
    freq_days: habit.reminder_config?.freq_days ?? habit.days ?? DEFAULT_CONFIG.freq_days,
  }))
  ...
  const persist = useCallback(async (next: ReminderConfig) => {
    ...
    await updateHabit(habit.id, { ...habit, reminder_config: next } as HabitWithConfig)
    setSaving(false)
    onRefresh()
  }, [habit, onRefresh])
  ...
  return (
    <div>
      <Pill
        label={pillLabel}
        variant={config.enabled ? 'goal' : 'habit'}
        size="sm"
        selected={open}
        onClick={() => setOpen(o => !o)}
        id="pill-lembrete"
