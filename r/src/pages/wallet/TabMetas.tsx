    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%', boxSizing: 'border-box' }}>
      ...
      <Modal open={!!deleteModal} onClose={() => setDeleteModal(null)}>
        <div style={{ padding: 24 }}>
          <h3 style={{ fontSize: 18, fontWeight: 500, color: 'var(--t1)', marginBottom: 12 }}>Excluir meta</h3>
...
const EmptyState = ({ onAction, isMobile }: { onAction: () => void; isMobile?: boolean }) => (
  <div style={{
    ...
    <div>
      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--t1)', marginBottom: 6 }}>Nenhuma meta ainda</div>
...
const GoalsOverview = ({ totals }: { totals: { saved: number; target: number; pct: number } }) => (
  <div style={{
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--t1)', lineHeight: 1 }}>
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 400, color: 'var(--t1)', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {goal.name}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, fontWeight: 500, fontFamily: 'var(--font-mono)', color }}>
            ...
            {done && (
              <span style={{ fontSize: 10, fontWeight: 500, padding: '1px 6px', background: '#dcfce7', border: '1.5px solid #22c55e', borderRadius: 99, color: '#15803d' }}>
                ✓ Concluída
              </span>
            )}
            {goal.deadline && !done && (
          ...
          {!isExpanded && (
            <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 500, color, flexShrink: 0 }}>{pct}%</span>
            </div>
          )}
        </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <div key={String(l)} style={{ flex: 1, padding: '10px 12px', background: 'var(--bg3)', borderRadius: 'var(--radius-sm)', display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 400, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{l}</span>
                <span style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-mono)', color: String(c) }}>{v}</span>
              </div>
            ))}
          </div>
        </div>
